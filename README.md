...
5. **Grant Lakebase permissions to your App's Service Principal**
   ...
   After deploying, you need to ensure your app has access to the necessary Lakebase tables for memory. The bundle deploys a Lakebase autoscaling instance (`postgres_projects` in `databricks.yml`) and configures it as an app resource automatically.

   **For autoscaling Lakebase instances**, use the role setup script to create a Postgres role for the app's service principal and grant schema-level permissions:

   ```bash
   # Get the SP client ID from your deployed app
   SP_CLIENT_ID=$(databricks apps get <app-name> --profile <profile> --output json | jq -r '.service_principal_client_id')

   # Get the project ID (listed after first deploy)
